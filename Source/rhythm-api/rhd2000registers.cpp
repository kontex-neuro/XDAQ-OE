...
Rhd2000Registers::Rhd2000Registers(double sampleRate)
{
    ...
    // adcBufferBias = 32;      // ADC reference buffer bias current (0 [highest current] - 63
    // [lowest current]); This value should be set according to ADC sampling rate; set in
    // setSampleRate()

    // muxBias = 40;            // ADC input MUX bias current (0 [highest current] - 63 [lowest
    // current]); This value should be set according to ADC sampling rate; set in setSampleRate()

    // muxLoad = 0;             // MUX capacitance load at ADC input (0 [min CL] - 7 [max CL]); LSB
    // = 3 pF Set in setSampleRate()
    ...
    twosComp = 0;  // two's complement ADC results (0 = unsigned offset representation; 1 = signed
                   // representation)
    absMode =
        0;  // absolute value mode (0 = normal output; 1 = output passed through abs(x) function)
    ...
    zcheckLoad =
        0;  // impedance testing dummy load (0 = normal operation; 1 = insert 60 pF to ground)
    setZcheckScale(ZcheckCs100fF);  // impedance testing scale factor (100 fF, 1.0 pF, or 10.0 pF)
    zcheckConnAll = 0;  // impedance testing connect all (0 = normal operation; 1 = connect all
                        // electrodes together)
    setZcheckPolarity(ZcheckPositiveInput);  // impedance testing polarity select (RHD2216 only) (0
                                             // = test positive inputs; 1 = test negative inputs)
...
// Define RHD2000 per-channel sampling rate so that certain sampling-rate-dependent registers are
// set correctly (This function does not change the sampling rate of the FPGA; for this, use
// Rhd2000EvalBoard::setSampleRate.)
...
// if enabled.
void Rhd2000Registers::setFastSettle(bool enabled) { ampFastSettle = (enabled ? 1 : 0); }
...
void Rhd2000Registers::enableAux1(bool enabled) { adcAux1En = (enabled ? 1 : 0); }

// Enable or disable ADC auxiliary input 2
void Rhd2000Registers::enableAux2(bool enabled) { adcAux2En = (enabled ? 1 : 0); }

// Enable or disable ADC auxiliary input 3
void Rhd2000Registers::enableAux3(bool enabled) { adcAux3En = (enabled ? 1 : 0); }

// Enable or disable DSP offset removal filter
void Rhd2000Registers::enableDsp(bool enabled) { dspEn = (enabled ? 1 : 0); }
...
void Rhd2000Registers::enableZcheck(bool enabled) { zcheckEn = (enabled ? 1 : 0); }

// Power up or down impedance checking DAC
void Rhd2000Registers::setZcheckDacPower(bool enabled) { zcheckDacPower = (enabled ? 1 : 0); }
...
{
    switch (polarity) {
    case ZcheckPositiveInput: zcheckSelPol = 0; break;
    case ZcheckNegativeInput: zcheckSelPol = 1; break;
    }
}
...
int Rhd2000Registers::getRegisterValue(int reg) const
{
    ...
    case 0:
        ...
        break;
    case 1: regout = (vddSenseEnable << 6) + adcBufferBias; break;
    case 2: regout = muxBias; break;
    case 3:
        regout = (muxLoad << 5) + (tempS2 << 4) + (tempS1 << 3) + (tempEn << 2) + (digOutHiZ << 1) +
                 digOut;
        break;
    case 4:
        regout = (weakMiso << 7) + (twosComp << 6) + (absMode << 5) + (dspEn << 4) + dspCutoffFreq;
        break;
    case 5:
        regout = (zcheckDacPower << 6) + (zcheckLoad << 5) + (zcheckScale << 3) +
                 (zcheckConnAll << 2) + (zcheckSelPol << 1) + zcheckEn;
        break;
    case 6: regout = zcheckDac; break;
    case 7: regout = zcheckSelect; break;
    case 8: regout = (offChipRH1 << 7) + rH1Dac1; break;
    case 9: regout = (adcAux1En << 7) + rH1Dac2; break;
    case 10: regout = (offChipRH2 << 7) + rH2Dac1; break;
    case 11: regout = (adcAux2En << 7) + rH2Dac2; break;
    case 12: regout = (offChipRL << 7) + rLDac1; break;
    case 13: regout = (adcAux3En << 7) + (rLDac3 << 6) + rLDac2; break;
    case 14:
...
uint32_t Rhd2000Registers::createRhd2000Command(Rhd2000CommandType commandType)
{
    switch (commandType) {
...
uint32_t Rhd2000Registers::createRhd2000Command(Rhd2000CommandType commandType, int arg1)
{
    switch (commandType) {
...
uint32_t Rhd2000Registers::createRhd2000Command(Rhd2000CommandType commandType, int arg1, int arg2)
{
    switch (commandType) {
    case Rhd2000CommandRegWrite:
        ...
        return 0x8000 + (arg1 << 8) +
               arg2;  // 10rrrrrrdddddddd; if the command is 'Register Write',
...
std::vector<uint32_t> Rhd2000Registers::createCommandListRegisterConfig(bool calibrate)
{
    ...
    // Don't program Register 6 (Impedance Check DAC) here; create DAC waveform in another command
    // stream
...
//
// Returns the length of the command list.
std::vector<uint32_t> Rhd2000Registers::createCommandListTempSensor()
{
    std::vector<uint32_t> commandList;
    ...
    commandList.push_back(
        createRhd2000Command(Rhd2000CommandConvert, 49));  // sample Temperature Sensor
    ...
    commandList.push_back(createRhd2000Command(Rhd2000CommandConvert, 34));  // sample AuxIn3
    commandList.push_back(
        createRhd2000Command(Rhd2000CommandConvert, 48));  // sample Supply Voltage Sensor
...
// this command list to coordinate with the 128-command list generated by
// createCommandListTempSensor().
//
// Returns the length of the command list.
std::vector<uint32_t> Rhd2000Registers::createCommandListUpdateDigOut()
{
    std::vector<uint32_t> commandList;
...
// amplitude (in DAC steps, 0-128) using the on-chip impedance testing voltage DAC.  If frequency is
// set to zero, a DC baseline waveform is created. Returns the length of the command list.
std::vector<uint32_t> Rhd2000Registers::createCommandListZcheckDac(double frequency,
                                                                   double amplitude)
{
    std::vector<uint32_t> commandList;
    int i, period, value;
    ...
    if (amplitude < 0.0 || amplitude > 128.0) {
        cerr << "Error in Rhd2000Registers::createCommandListZcheckDac: Amplitude out of range."
             << endl;
        return commandList;
    }
    if (frequency < 0.0) {
        cerr << "Error in Rhd2000Registers::createCommandListZcheckDac: Negative frequency not "
                "allowed."
             << endl;
        return commandList;
    ...
    if (frequency == 0.0) {
        ...
        if (period > MaxCommandLength) {
            cerr << "Error in Rhd2000Registers::createCommandListZcheckDac: Frequency too low."
                 << endl;
            return commandList;
        } else {
            t = 0.0;
